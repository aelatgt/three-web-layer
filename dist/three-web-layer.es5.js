import { Vector3, Object3D, Mesh, MeshBasicMaterial, MeshDepthMaterial, RGBADepthPacking, Raycaster, Ray, VideoTexture, Texture, LinearFilter, PlaneGeometry, Matrix4 } from 'three';
 ...
 */
class WebLayer3D extends Object3D {
    constructor(element, options = {}, parentLayer = undefined, _level = 0) {
        super();
        this.options = options;
        this.parentLayer = parentLayer;
        this._level = _level;
        this.content = new Object3D();
        this.mesh = new Mesh(WebLayer3D.GEOMETRY, new MeshBasicMaterial({
            transparent: true,
            alphaTest: 0.001,
            opacity: 0
        }));
    ...
    static async _scheduleRefresh(rootLayer) {
        ...
        if (queue.length === 0 || rootLayer.options.autoRasterize === false)
            return;
    ...
    _refreshMesh() {
        ...
        const index$$1 = this.parentLayer ? this.parentLayer.childLayers.indexOf(this) : 0;
        mesh.renderOrder = this.level + index$$1 * 0.0001;
    }
    ...
    async rasterize() {
        ...
        for (const stateKey in states) {
            ...
            for (let hover = 0; hover <= hoverDepth; hover++) {
                ...
                const texture = state.texture || new Texture(document.createElement('canvas'));
                if (stateKey)
