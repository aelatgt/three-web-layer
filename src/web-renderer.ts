...
export class WebRenderer {
  ...
  static _init() {
    ...
    addCSSRule(
      sheet,
      `[data-layer-rendering-parent]`,
      'transform: none;left: 0;top: 0;margin: 0;border:0;border-radius:0;height:100%;padding:0;position:static;text-align:left;display:block;background:rgba(0,0,0,0);box-shadow:none',
      i++
    )
    addCSSRule(
      sheet,
      `[data-layer-rendering-parent]::before, [data-layer-rendering-parent]::after`,
      'content:none; box-shadow:none;',
      i++
    )
  ...
  static async generateEmbeddedCSS(url, css): Promise<string> {
    ...
    while ((found = regEx.exec(css))) {
      promises.push(
        this.getDataURL(new URL(found[1], url)).then(url => {
