import { Object3D, Mesh, MeshBasicMaterial, DoubleSide, Vector3, Texture, Math as Math$1, LinearFilter, PlaneGeometry } from 'three';
import os from 'os';
...
class WebLayer3D extends Object3D {
    constructor(element, options = {}, rootLayer = undefined, level = 0) {
        ...
        this.defaultContentScale = new Vector3();
        this.textures = {
    ...
    transitionEntryExit(alpha) {
        ...
        if (this.needsRemoval) {
            ...
            if ('opacity' in material && material.opacity < 1) {
                material.opacity = Math$1.lerp(material.opacity, 1, alpha);
                material.needsUpdate = true;
            }
        }
    }
    ...
    async refresh(forceClone = false) {
        ...
        for (const state of this._states) {
            if (!this.textures[state]) {
                this.textures[state] = new Texture(document.createElement('canvas'));
            }
        }
