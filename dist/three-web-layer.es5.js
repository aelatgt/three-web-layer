import { Object3D, Texture, Mesh, MeshBasicMaterial, DoubleSide, Vector3, Raycaster, Math as Math$1, LinearFilter, PlaneGeometry } from 'three';
import os from 'os';
...
class WebLayer3D extends Object3D {
    constructor(element, options = {}, rootLayer = undefined, level = 0) {
        ...
        this.textures = {
            default: new Texture(document.createElement('canvas'))
        };
        ...
        this.defaultContentPosition = new Vector3();
        this.defaultContentScale = new Vector3(1, 1, 1);
    ...
    transitionEntryExit(alpha) {
        ...
        if (this.needsRemoval) {
            ...
            if ('opacity' in material && material.opacity < 1) {
                material.opacity = Math.min(Math$1.lerp(material.opacity, 1, alpha), 1);
                material.needsUpdate = true;
            }
        }
    }
    ...
    async refresh(forceClone = false) {
        ...
        for (const state of this._states) {
            if (!this.textures[state]) {
                ...
                this.textures[state] = new Texture(canvas);
            }
        }
    ...
    _updateTexture(canvas, state = 'default') {
        ...
        stateTexture.minFilter = LinearFilter;
